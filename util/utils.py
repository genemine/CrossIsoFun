...
def cal_sample_weight_wrong(labels, num_class, use_sample_weight=True):
...
def cal_sample_weight_right(labels, num_class, use_sample_weight=True):
    if not use_sample_weight:
        return np.ones(len(labels)) / len(labels)
    count = np.zeros(num_class)
    for i in range(num_class):
...
# def gen_adj_mat_tensor(data, parameter, gpu_ids, metric="cosine"):
#     assert metric == "cosine", "Only cosine distance implemented"
#     dist = cosine_distance_torch(data, data)
...
def gen_adj_mat_tensor(data, parameter, i, metric="cosine"):
    ...
    adj = F.normalize(adj + I, p=1)
    # torch.save(adj, 'train_save_tensor'+str(i)+'.pt')
    adj = to_sparse(adj)
...
# def gen_test_adj_mat_tensor(data, trte_idx, parameter, gpu_ids, metric="cosine"):
#     assert metric == "cosine", "Only cosine distance implemented"
#     adj = torch.zeros((data.shape[0], data.shape[0]))
#     adj = adj.cuda(gpu_ids[0])
...
def gen_test_adj_mat_tensor(data, trte_idx, parameter, i, metric="cosine"):
    ...
    adj[mask] = adj[mask]+adj_T[mask]-adj[mask]
    adj = F.normalize(adj + I, p=1)
    # torch.save(adj, 'test_save_tensor'+str(i)+'.pt')
    adj = to_sparse(adj)

    return adj


def load_adj_mat_tensor(i):
    adj = torch.load('train_save_tensor' + str(i) + '.pt')
    # print(torch.sum((adj > 0).float()))
    print(adj.shape)
    print(adj)
    adj = to_sparse(adj)
...
def load_test_adj_mat_tensor(i):
    adj = torch.load('test_save_tensor' + str(i) + '.pt')
    # print(torch.sum((adj > 0).float(),dim=1))
    print(adj.shape)
    print(adj)
    adj = to_sparse(adj)
